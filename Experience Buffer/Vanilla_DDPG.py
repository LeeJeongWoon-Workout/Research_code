...
class ReplayBuffer:
    ...
    def sample_batch(self) -> Dict[str,np.ndarray]:
        ...
        return dict(obs=self.obs_buf[idxs],acts=self.acts_buf[idxs],rews=self.rews_buf[idxs],next_obs=self.next_obs_buf[idxs],done=self.done_buf[idxs])
...
class DDPGAgent_van:
    ...
    def train(self,num_frames:int,plotting_interval:int=200) -> np.ndarray:
        ...
        for frame_idx in range(num_frames):
            ...
            if (frame_idx+1)%5==0:
                result_lst.append(result_score/5)
                result_score=0

            if len(self.memory)>=self.batch_size and len(self.memory)>=1000:
        ...
        self.env.close()
        result_lst=np.array(result_lst,dtype=np.float32)
        return result_lst
