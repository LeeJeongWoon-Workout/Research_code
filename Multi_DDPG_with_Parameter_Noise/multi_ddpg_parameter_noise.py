...
from utils import OrnsteinUhlenbeckProcess,AdaptiveParamNoiseSpec,ddpg_distance_metric
...
class multi_parameter_noise_ddpg:

    def __init__(self,num_episode:int,batch_size):
        self.num_episode=num_episode
        self.batch_size=batch_size

        self.ddpg1=DDPG(num_episode)
        self.ddpg2=DDPG(num_episode)
        self.ddpg3=DDPG(num_episode)
            ...
            while True:
                ...
                if not done1:
                    ...
                    if len(self.memory)>=self.batch_size and len(self.hmemory)>=self.batch_size:
                        ddpg1.train(self.memory,self.hmemory,batch_size=self.batch_size)
                ...
                if not done2:
                    ...
                    if len(self.memory)>=self.batch_size and len(self.hmemory)>=self.batch_size:
                        ddpg2.train(self.memory,self.hmemory,batch_size=self.batch_size)
                ...
                if not done3:
                    ...
                    if len(self.memory)>=self.batch_size and len(self.hmemory)>=self.batch_size:
                        ddpg3.train(self.memory,self.hmemory,batch_size=self.batch_size)
