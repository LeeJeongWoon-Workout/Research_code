...
if __name__ == "__main__":
    ...
    batch_size=32
    epoch=5
    result_van=np.zeros([int(num_frames)],dtype=np.float32)
    result_epi=np.zeros([int(num_frames)],dtype=np.float32)
    result_epi_mo=np.zeros([int(num_frames)],dtype=np.float32)
    result_epi_m=np.zeros([int(num_frames)],dtype=np.float32)
    result_my=np.zeros([int(num_frames)],dtype=np.float32)
